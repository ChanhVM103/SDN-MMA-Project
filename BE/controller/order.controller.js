...
const createOrder = async (req, res) => {
    try {
        ...
        // Validate
        if (!restaurantId) {
            return res.status(400).json({ success: false, message: "Vui lòng chọn nhà hàng" });
        }
        ...
        if (!restaurant.isOpen) {
            return res.status(400).json({ success: false, message: "Nhà hàng hiện đang đóng cửa" });
        }
...
const getOrderStats = async (req, res) => {
    try {
        ...
        const [total, todayCount, pending, confirmed, preparing, delivering, delivered, cancelled, revenue] =
            await Promise.all([
