...
export default function RestaurantAddProduct() {
    ...
    React.useEffect(() => {
        const fetchRest = async () => {
            if (token) {
                const res = await restaurantAPI.getMyRestaurant(token).catch(() => null);
                if (res?.success) setMyRestaurant(res.data);
            }
        };
        fetchRest();
    }, [token]);
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            const restId = myRestaurant?._id || myRestaurant?.id;
            if (!restId) {
                throw new Error('Bạn cần có nhà hàng được cấp phép trước khi thêm sản phẩm!');
            }

            const res = await productAPI.createProduct(token, restId, productData);
            if (res.success) {
                const msg = 'Thêm sản phẩm thành công!';
            ...
            <View style={s.container}>
                ...
                <View style={s.header}>
                    ...
                    <View style={{ alignItems: 'center' }}>
                        <Text style={s.headerTitle}>Thêm sản phẩm</Text>
                        {myRestaurant && (
                            <Text style={{ fontSize: 12, color: AppColors.primary, fontWeight: '600' }}>
                                Cho: {myRestaurant.name}
                            </Text>
                        )}
                    </View>
